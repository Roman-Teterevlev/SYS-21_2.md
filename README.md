...
frontend example  # секция фронтенд
        ...
        use_backend web_server1 if ACL_example1.local
        acl ACL_example2.local hdr(host) -i example2.local
        use_backend web_server2 if ACL_example2.local

backend web_server1    # секция бэкенд
        mode http
        balance roundrobin
        option httpchk
        http-check send meth GET uri /index.html
...
backend web_server2    # секция бэкенд
        mode http
        balance roundrobin
        option httpchk
        http-check send meth GET uri /index.html
